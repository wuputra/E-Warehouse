...
if (isset($_POST['getSimpan'])) {
	...
	if ($kode_barang == " " || $nama_barang == " " || $merek_barang == " " || $distributor == " " || $harga == " " || $stok == " " || $ket == " ") {
		...
		if ($harga < 0 || $stok < 0) {
			...
			if ($_FILES['foto']['name'] == "") {
				$value = "kd_barang='$kode_barang',nama_barang='$nama_barang',kd_merek='$merek_barang',kd_distributor='$distributor',tanggal_masuk='$waktu',harga_barang='$harga',stok_barang='$stok',spesifikasi='$spesifikasi',keterangan='$ket'";
				$response = $br->update($table, $value, "kd_barang", $_GET['id'], "?page=viewBarangHardware");
			} else {
				$response = $br->validateImage();
				if ($response['types'] == "true") {
					$value = "kd_barang='$kode_barang',nama_barang='$nama_barang',kd_merek='$merek_barang',kd_distributor='$distributor',tanggal_masuk='$waktu',harga_barang='$harga',stok_barang='$stok',spesifikasi='$spesifikasi',keterangan='$ket',gambar='$response[image]'";
					$response = $br->update($table, $value, "kd_barang", $_GET['id'], "?page=viewBarangHardware");
				} else {
						...
						<div class="card">
							...
							<div class="card-footer">
								<button name="getSimpan" class="btn btn-primary"><i class="fa fa-download"></i>
									Update</button>
								<a href="?page=viewBarangHardware" class="btn btn-danger"><i class="fa fa-repeat"></i>
									Kembali</a>
							</div>
						</div>
					</form>
