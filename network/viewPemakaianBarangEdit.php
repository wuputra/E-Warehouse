...
if (isset($_GET['edit'])) {
	$id = $_GET['id'];
	$dataEdit = $trans->selectWhere("table_pretransaksi", "kd_pretransaksi", $id);
	$dataR = $trans->selectWhere("table_barang", "kd_barang", $dataEdit['kd_barang']);
...
if (isset($_POST['btnUpdate'])) {
	...
	if (!preg_match("/^\d{4}-\d{2}-\d{2}$/", $tanggal_keluar) || strtotime($tanggal_keluar) === false) {
		...
		$barangSekarang = $trans->selectWhere("table_barang", "kd_barang", $kd_barang);
		$stokSekarang = $barangSekarang['stok_barang'];

		// Ambil jumlah lama dari pretransaksi (yang sedang diedit)
		$dataLama = $trans->selectWhere("table_pretransaksi", "kd_pretransaksi", $kd_pretransaksi);
		$jumlahLama = $dataLama['jumlah'];

		// Hitung stok baru (mengembalikan jumlah lama, lalu kurangi dengan jumlah baru)
		...
		if ($stokBaru < 0) {
			...
			$response = $trans->update("table_pretransaksi", $value, $where, $whereValues, "?page=viewPemakaianBarang");
		}
	}
}
?>
					...
					<form method="post" enctype="multipart/form-data">
						<div class="card">
							...
							<div class="card-footer">
								...
								<a href="?page=viewPemakaianBarang" class="btn btn-danger"><i class="fa fa-repeat"></i>
									Kembali</a>
							</div>
				...
				<table class="table table-hover table-bordered" id="sampleTable">
					...
					<tbody>
						<?php foreach ($barangs as $brs) { ?>
							<tr>
								<td><a
										href="pageNetwork.php?page=addPemakaianBarang&getItem&id=<?php echo $brs['kd_barang'] ?>"><?php echo $brs['kd_barang'] ?></a>
								</td>
